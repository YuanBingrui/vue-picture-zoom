<template>
  <div class="picture-zoom-box" :ref="id" @mousemove="mousemove" @mouseover="mouseover" @mouseleave="mouseleave">
    <img class="zoom-img" :src="imgUrl" alt="picture-zoom">
  ...
  methods: {
    ...
    mousemove(event) {
      ...
      function offset(curEle) {
        var totalLeft = 0, totalTop = 0, parentEle = curEle.offsetParent;
        //首先加上本身的左偏移和上偏移
      ...
      let startX = curPosition.x - (imgWrap.left - document.documentElement.scrollLeft),
          startY = curPosition.y - (imgWrap.top - document.documentElement.scrollTop);
